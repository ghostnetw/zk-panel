...
CreateThread(function()
    Wait(50)
    local DM = GetResourceKvpInt("zk-admin:setting:darkmode")
    if DM == 1 then DM = true elseif DM == 0 then DM = false end
    local ST = GetResourceKvpInt("zk-admin:setting:seethrough")
    if ST == 1 then ST = true elseif ST == 0 then ST = false end
    local NT = GetResourceKvpInt("zk-admin:setting:notifications")
    if NT == 1 then NT = true elseif NT == 0 then NT = false end
    local theme = GetResourceKvpInt("zk-admin:setting:theme")
...
RegisterCommand("*adminpanel", function()
    --TriggerServerEvent("zk-admin:server:RefreshMenu", true)
    --Wait(200)
    TriggerServerEvent("zk-admin:server:RequestPanel")
end, false)
RegisterKeyMapping("*adminpanel", "(zkadmin)", "keyboard", Config.AdminPanelKey)

RegisterCommand("*noclip", function()
    TriggerServerEvent("zk-admin:server:RequestNoClip")
end, false)
RegisterKeyMapping("*noclip", Lang:t("commands.noclip").."(zkadmin)", "keyboard", Config.NoClipKey)

RegisterCommand("*showNames", function()
    if Config.EnableNames then
        if not Config.AllPlayersUseNames then
            Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
                AdminPanel.DisplayingNames = not AdminPanel.DisplayingNames
                if AdminPanel.DisplayingNames then
                    ShowNames = true
                    toggleNames()
                    -- TriggerServerEvent("zk-admin:server:GetPlayersForBlips")
                else
                    ShowNames = false
                    toggleNames()
                    -- TriggerServerEvent("zk-admin:server:GetPlayersForBlips")
...
RegisterKeyMapping("*showNames", Lang:t("commands.showNames").."(zkadmin)", "keyboard", Config.ShowNamesKey)
...
RegisterNetEvent("zk-admin:client:ToggleNoClip", function()
    ...
    if AdminPanel.NewNoClip or AdminPanel.NoClip then
        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.enabled").."</strong>"..Lang:t("alerts.noclipEnabled"))
    else
        TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.disabled").."</strong>"..Lang:t("alerts.noclipDisabled"))
    end
end)

RegisterNetEvent("zk-admin:client:OpenMenu", function(playerList, ServerInformation, MaxPlayers, Version, hasPerms, Role)
    SendNUIMessage({
        action = "open",
...
RegisterNetEvent("zk-admin:client:RefreshMenu", function(playerList, silent)
    SendNUIMessage({
        action = "refresh",
        name = GetPlayerName(PlayerId()),
        playerlist = playerList,
    ...
    if not silent then
        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.playerListRefreshed"))
    end
end)

RegisterNetEvent("zk-admin:client:ViewPlayer", function(online, playerData)
    if online then
        SendNUIMessage({
...
RegisterNetEvent("zk-admin:client:ResetMenu", function()
    SendNUIMessage({
        action = "noperms"
    })
end)

RegisterNetEvent("zk-admin:client:WarnPlayer", function(warnedBy, reason)
    SendNUIMessage({
        action = "showwarning",
        by = warnedBy,
        reason = reason
...
RegisterNUICallback("GiveItem", function(data)
    TriggerServerEvent("zk-admin:server:GiveItem", data.Id, data.Item, data.Amount)
end)

RegisterNUICallback("SpawnVehicle", function(data)
    TriggerServerEvent("zk-admin:server:RequestVehicleSpawn", data.Vehicle)
end)

RegisterNUICallback("ReportReply", function(data)
    if data ~= nil then
        TriggerServerEvent("zk-admin:server:ReportReply", data)
    end
end)
...
RegisterNetEvent("zk-admin:client:ShowPanelAlert", function(type, text)
...
RegisterNetEvent("zk-admin:client:ShowReportAlert", function(title, text)
    PlaySoundFrontend(-1, "Cancel", "GTAO_Exec_SecuroServ_Warehouse_PC_Sounds", true)
    SendNUIMessage({
...
RegisterNetEvent("zk-admin:client:ForceReloadResources", function()
    TriggerServerEvent("zk-admin:server:RequestResourcePageInfo")
end)

RegisterNetEvent("zk-admin:client:ReceiveJobPageInfo", function(JobPageInfo)
...
RegisterNetEvent("zk-admin:client:ReceiveGangPageInfo", function(GangPageInfo)
...
RegisterNetEvent("zk-admin:client:ReceiveResourcePageInfo", function(ResourcePageInfo)
...
RegisterNetEvent("zk-admin:client:ReceiveBansInfo", function(BansInfo)
...
RegisterNetEvent("zk-admin:client:ReceiveReportsInfo", function(ReportsInfo)
...
RegisterNetEvent("zk-admin:client:ReceiveAdminChat", function(AdminChat)
...
RegisterNetEvent("zk-admin:client:ReceiveVehiclesInfo", function(VehiclesInfo)
...
RegisterNetEvent("zk-admin:client:ReceiveLeaderboardInfo", function(money, vehicles)
...
RegisterNetEvent("zk-admin:client:ReceiveItemsInfo", function(ItemsInfo)
...
RegisterNetEvent("zk-admin:client:ReceiveCharacters", function(players)
...
RegisterNetEvent("zk-admin:client:ReceiveCurrentLogs", function(currentLogs)
...
RegisterNetEvent("zk-admin:client:ToggleBlips", function()
    --COME BACK HERE
end)

RegisterNetEvent("zk-admin:client:ViewWarnings", function(warnings)
    DebugTrace("[zk-admin:client:ViewWarnings] Received warnings")
    SendNUIMessage({
        action = "viewwarnings",
        warnings = warnings,
    })
...
RegisterNetEvent("zk-admin:client:SetPedModel", function(model)
    if model then
...
RegisterNetEvent("zk-admin:client:DeleteAllEntities", function(entityType)
...
RegisterNetEvent("zk-admin:client:RequestSpectate", function(playerServerId, tgtCoords, name)
...
RegisterNetEvent("zk-admin:client:RequestInventory", function(TargetId)
    Compat.OpenInventory(TargetId)
end)
  
RegisterNetEvent("zk-admin:client:setLivery", function(livery)
...
RegisterNetEvent("zk-admin:client:SetPosition", function(coords)
    local ped = PlayerPedId()
    SetEntityCoords(ped, coords.x, coords.y, coords.z)
end)

RegisterNetEvent("zk-admin:client:Freeze", function()
...
RegisterNetEvent("zk-admin:client:SendReport", function(name, src, msg, subject)
    TriggerServerEvent("zk-admin:server:SendReport", name, src, msg, subject)
end)

RegisterNetEvent("zk-admin:client:SaveCar", function(senderId)
    local ped = PlayerPedId()
    local veh = GetVehiclePedIsIn(ped)
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if QBCore.Shared.Vehicles[vehname] ~= nil and next(QBCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent("zk-admin:server:SaveCar", props, QBCore.Shared.Vehicles[vehname], `veh`, plate, senderId)
        else
...
RegisterNetEvent("zk-admin:client:SetModel", function(skin)
    local ped = PlayerPedId()
...
RegisterNetEvent("zk-admin:client:MassDespawn", function ()
    for vehicle in EnumerateVehicles() do
        if (not IsPedAPlayer(GetPedInVehicleSeat(vehicle, -1))) then 
            SetVehicleHasBeenOwnedByPlayer(vehicle, false) 
            SetEntityAsMissionEntity(vehicle, false, false) 
...
RegisterNetEvent("zk-admin:client:MassPedDespawn", function ()
    for ped in EnumeratePeds() do
        if (not IsPedAPlayer(ped)) then 
            SetEntityAsMissionEntity(ped, false, false) 
            DeleteEntity(ped)
...
RegisterNetEvent("zk-admin:client:MassObjDespawn", function ()
    for object in EnumerateObjects() do
...
RegisterNetEvent("zk-admin:client:MassEverythingDespawn", function ()
...
RegisterNetEvent("zk-admin:client:SetNuiFocus", function(focus, mouse)
    SetNuiFocus(focus, mouse)
end)

RegisterNetEvent("zk-admin:client:ReceiveServerMetrics", function(serverMetrics)
...
RegisterNUICallback("close", function()
    SetNuiFocus(false, false)
    TriggerServerEvent("zk-admin:server:ClosePanel")
    DebugTrace("NUICallback: close")
end)

RegisterNUICallback("LoadJobInfo", function()
    TriggerServerEvent("zk-admin:server:RequestJobPageInfo")
    DebugTrace("NUICallback: LoadJobInfo")
end)

RegisterNUICallback("LoadGangInfo", function()
    TriggerServerEvent("zk-admin:server:RequestGangPageInfo")
...
RegisterNUICallback("ResourceAction", function(info)
    ...
    if info["action"] == "start" or info["action"] == "stop" or info["action"] == "restart" then
        TriggerServerEvent("zk-admin:server:ResourceAction", currentResource, info["action"])
    end
    DebugTrace("NUICallback: ResourceAction ["..info["action"].."]")
end)

RegisterNetEvent("zk-admin:client:OpenSkinMenu1", function()
...
RegisterNUICallback("Action", function(info)
    ...
    if info["action"] == "uncuffSelf" then
        Compat.UncuffSelf()
        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.uncuffed"))
    elseif info["action"] == "revive" then
        TriggerServerEvent("zk-admin:server:RevivePlayer", currentPlayer)
    elseif info["action"] == "goto" then
        TriggerServerEvent("zk-admin:server:GotoPlayer", currentPlayer)
    elseif info["action"] == "bring" then
        local target = GetPlayerPed(currentPlayer)
        local plyCoords = GetEntityCoords(PlayerPedId())
        TriggerServerEvent("zk-admin:server:BringPlayer", currentPlayer, plyCoords)
    elseif info["action"] == "sendtolegion" then
        TriggerServerEvent("zk-admin:server:SetPosition", currentPlayer, 215.75, -804.26, 30.81)
        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.senToLegion"))
    elseif info["action"] == "sendtopillbox" then
        TriggerServerEvent("zk-admin:server:SetPosition", currentPlayer, 299.01, -577.48, 43.26)
        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.sentToPillbox"))
    elseif info["action"] == "sendtolsc" then
        TriggerServerEvent("zk-admin:server:SetPosition", currentPlayer, -366.58, -126.01, 38.69)
        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.sentToCustoms"))
    elseif info["action"] == "sendtomrpd" then
        TriggerServerEvent("zk-admin:server:SetPosition", currentPlayer, 415.41, -993.4, 29.38)
        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.sentToMRPD"))
    elseif info["action"] == "sendtosandy" then
        TriggerServerEvent("zk-admin:server:SetPosition", currentPlayer, 1963.56, 3735.19, 32.2)
        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.sentToSandy"))
    elseif info["action"] == "sendtograpeseed" then
        TriggerServerEvent("zk-admin:server:SetPosition", currentPlayer, 1692.89, 4942.49, 42.32)
        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.sentToGrapeseed"))
    elseif info["action"] == "sendtopaleto" then
        TriggerServerEvent("zk-admin:server:SetPosition", currentPlayer, 125.64, 6611.6, 31.86)
        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.sentToPaleto"))
    elseif info["action"] == "sendtolsia" then
        TriggerServerEvent("zk-admin:server:SetPosition", currentPlayer, -1021.81, -2701.25, 13.76)
        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.sentToLSA"))
    elseif info["action"] == "kill" then
        TriggerServerEvent("zk-admin:server:KillPlayer", currentPlayer)
    elseif info["action"] == "cuff" then
        TriggerServerEvent("zk-admin:server:CuffPlayer", currentPlayer)
    elseif info["action"] == "save" then
        TriggerServerEvent("zk-admin:server:SavePlayer", currentPlayer)
    elseif info["action"] == "clothing" then
        TriggerServerEvent("zk-admin:server:OpenSkinMenu", currentPlayer)
    elseif info["action"] == "sclothing" then
        SendNUIMessage({
            action = "close"
        })
        SetNuiFocus(false, false)
        TriggerServerEvent("zk-admin:server:OpenSkinMenu", GetPlayerServerId(PlayerId()))
    elseif info["action"] == "feed" then
        TriggerServerEvent("zk-admin:server:FeedPlayer", currentPlayer)
        Citizen.Wait(50)
        TriggerServerEvent("zk-admin:server:RefreshMenu", true)
    elseif info["action"] == "stress" then
        TriggerServerEvent("zk-admin:server:RelieveStress", currentPlayer)
        Citizen.Wait(50)
        TriggerServerEvent("zk-admin:server:RefreshMenu", true)
    elseif info["action"] == "openinv" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            SetNuiFocus(false, false)
            TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.openingPlayerInv"))
            TriggerEvent("zk-admin:client:RequestInventory", currentPlayer)
        end, "openinventory")
    elseif info["action"] == "clearinv" then
        TriggerServerEvent("zk-admin:server:ClearInventory", currentPlayer)
    elseif info["action"] == "screenshot" then
        TriggerServerEvent("zk-admin:server:ScreenshotSubmit", currentPlayer)
    elseif info["action"] == "spectate" then
        TriggerServerEvent("zk-admin:server:RequestSpectate", currentPlayer)
        SendNUIMessage({
            action = "close"
        })
        SetNuiFocus(false, false)
    elseif info["action"] == "freeze" then
        TriggerServerEvent("zk-admin:server:Freeze", currentPlayer)
    elseif info["action"] == "revives" then
        TriggerServerEvent("zk-admin:server:RevivePlayer", GetPlayerServerId(PlayerId()))
    elseif info["action"] == "sendback" then
        TriggerServerEvent("zk-admin:server:SendPlayerBack", currentPlayer)
    elseif info["action"] == "goback" then
        TriggerServerEvent("zk-admin:server:SendBackSelf")
    elseif info["action"] == "tpm" then
        if IsWaypointActive() then
            ...
            if DoesBlipExist(WaypointHandle) then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.tpm"))
            else
                ESX.ShowNotification("Select Waypoint")
            end
        else
            TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.noMarker"))
        end
    elseif info["action"] == "setpedmodel" then
        if info["model"] then
            TriggerServerEvent("zk-admin:server:SetPedModel", currentPlayer, info["model"])
        end
    elseif info["action"] == "invisible" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if not AdminPanel.isInvisible then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.enabled").."</strong> "..Lang:t("alerts.invis"))
            else
                SetEntityVisible(PlayerPedId(), true, false)
                AdminPanel.isInvisible = false
                TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.disabled").."</strong> "..Lang:t("alerts.vis"))
            end
        end, "invisibility")
    elseif info["action"] == "togradar" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if AdminPanel.DisplayingRadar then
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.enabled").."</strong> "..Lang:t("alerts.radarOn"))
            else
                TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.disabled").."</strong> "..Lang:t("alerts.radarOff"))
            end
        end, "forceradar")
    elseif info["action"] == "togblips" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if AdminPanel.DisplayingBlips then
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.enabled").."</strong> "..Lang:t("alerts.blipsOn"))
            else
                TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.disabled").."</strong> "..Lang:t("alerts.blipsOff"))
            end
        end, "playerblips")
    elseif info["action"] == "tognames" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if AdminPanel.DisplayingNames then
                ...
                -- TriggerServerEvent("zk-admin:server:GetPlayersForBlips")
                toggleNames()
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.enabled").."</strong> "..Lang:t("alerts.NamesOn"))
            else
                ShowNames = false
                -- TriggerServerEvent("zk-admin:server:GetPlayersForBlips")
                toggleNames()
                TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.disabled").."</strong> "..Lang:t("alerts.NamesOff"))
            end
        end, "playerblips")
    elseif info["action"] == "togspeed" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if AdminPanel.FastSpeed then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.enabled").."</strong> "..Lang:t("alerts.RunOn"))
            else
                SetRunSprintMultiplierForPlayer(PlayerId(), 1.0)
                SetSwimMultiplierForPlayer(PlayerId(), 1.0)
                TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.disabled").."</strong> "..Lang:t("alerts.RunOff"))
            end
        end, "fastrun")
    elseif info["action"] == "god" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if not AdminPanel.GodMode then
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.enabled").."</strong> "..Lang:t("alerts.GodOn"))
                godModeChange()
            else
                TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.disabled").."</strong> "..Lang:t("alerts.GodOff"))
                godModeChange()
            end
        end, "godmode")
        ...
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if vehicle then
                if IsVehicleSeatFree(vehicle,-1) then
                    ...
                    TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.EnterVehicle"))
                else
                    TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.VehicleFull"))
                end
            end
        end, "setmedriver")
    ...
    elseif info["action"] == "clearblood" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.bloodCleared"))
        end, "clearblood")
    elseif info["action"] == "wetclothes" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            SetPedWetnessHeight(PlayerPedId(), 2.0)
            TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.clothesWet"))
        end, "wetclothes")
    elseif info["action"] == "dryclothes" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ClearPedWetness(PlayerPedId())
            TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.clothesDry"))
        end, "dryclothes")
        ...
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if entity then
                if distance < 10.0 then
                    ...
                    TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.error").."</strong>"..Lang:t("alerts.vehicleFar"))
                end
            end
            ...
            if entity then
                if not IsPedAPlayer(entity) then
                    if distance < 10.0 then
                        ...
                        TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.error").."</strong>"..Lang:t("alerts.pedFar"))
                    end
                else
                    TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.error").."</strong>"..Lang:t("alerts.pedIsPlayer"))
                end
            end
        ...
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if entity then
                if distance < 10.0 then                
                    ...
                    TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.error").."</strong>"..Lang:t("alerts.objectFar"))
                end
            end
    ...
    elseif info["action"] == "repairv" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if vehicle then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.repairedVehicle"))
            end
        end, "repairvehicle")
    elseif info["action"] == "wash" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if vehicle then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.washedVehicle"))
            end
        end, "washvehicle")
    elseif info["action"] == "hotwire" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if vehicle then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.hotwireVehicle"))
            end
        end, "hotwirevehicle")
    elseif info["action"] == "lockv" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if vehicle then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.lockedVehicle"))
            end
        end, "lockvehicle")
    elseif info["action"] == "unlockv" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if vehicle then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.lockedVehicle"))
            end
        end, "unlockvehicle")
    elseif info["action"] == "spawnvehicle" then
        if info["model"] then
            ...
            SetNuiFocus(false, false)
            TriggerServerEvent("zk-admin:server:RequestVehicleSpawn", info["model"])
        end
    elseif info["action"] == "maxpupgrades" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if vehicle then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.vehicleMaxPerformance"))
                SendNUIMessage({
                    action = "close"
                })
                SetNuiFocus(false, false)
        ...
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if vehicle then
                if distance < 10.0 then
                    ...
                    TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.vehicleRandomVisual"))
                    SendNUIMessage({
                        action = "close"
                    })
                    SetNuiFocus(false, false)
            ...
            if vehicle then
                if distance < 10.0 then
                    if info["livery"] then
                        ...
                        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.liverySet").." "..GetLabelText(GetLiveryName(vehicle, tonumber(info["livery"])))..".")
                        SendNUIMessage({
                            action = "close"
                        })
                        SetNuiFocus(false, false)
            ...
            if info["primarycolor"] and info["secondarycolor"] then
                ...
                if vehicle then
                    if distance < 10.0 then
                        ...
                        TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.colorSet"))
                        SendNUIMessage({
                            action = "close"
                        })
                        SetNuiFocus(false, false)
    ...
    elseif info["action"] == "filltank" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if vehicle then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.fuelSet"))
            end
        end, "fillgastank")
    elseif info["action"] == "kick" then       
        TriggerServerEvent("zk-admin:server:KickPlayer", currentPlayer, info["reason"])
    elseif info["action"] == "warn" then       
        TriggerServerEvent("zk-admin:server:WarnPlayer", currentPlayer, info["reason"], info["citizenid"])
    elseif info["action"] == "checkwarn" then
        TriggerServerEvent("zk-admin:server:ViewWarnings", currentPlayer, info["citizenid"])
    elseif info["action"] == "ban" then       
        info["timeamt"] = info["timeamt"] * 60 * 60
        TriggerServerEvent("zk-admin:server:BanPlayer", currentPlayer, info["timeamt"], info["reason"], info["citizenid"])
    elseif info["action"] == "givecash" or info["action"] == "removecash" or info["action"] == "givebank" or info["action"] == "removebank" then
        ...
        TriggerServerEvent("zk-admin:server:MonetaryAction", currentPlayer, info["action"], cashAmount)
        Citizen.Wait(50)
        TriggerServerEvent("zk-admin:server:RefreshMenu", true)
    elseif info["action"] == "setjob" then
        TriggerServerEvent("zk-admin:server:SetJob", currentPlayer, info["jobname"], info["jobgrade"])
        Citizen.Wait(50)
        TriggerServerEvent("zk-admin:server:RefreshMenu", true)
    elseif info["action"] == "giveitem" then
        TriggerServerEvent("zk-admin:server:GiveItem", currentPlayer, info["itemname"], info["itemamount"])
    elseif info["action"] == "setgang" then
        TriggerServerEvent("zk-admin:server:SetGang", currentPlayer, info["gangname"], info["ganggrade"])
        Citizen.Wait(50)
        TriggerServerEvent("zk-admin:server:RefreshMenu", true)
    elseif info["action"] == "loadipl" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if info["ipl"] ~= "" and info["ipl"] ~= nil then
                RequestIpl(info["ipl"])
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.iplReqested"))
            end
        end, "ipl")
    elseif info["action"] == "unloadipl" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if info["ipl"] ~= "" and info["ipl"] ~= nil then
                RemoveIpl(info["ipl"])
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.unloadIPL"))
            end
        end, "ipl")
    elseif info["action"] == "setViewDistance" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if hasPermission then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.distanceChanged"))
            end
        end, "setViewDistance")
    elseif info["action"] == "copyEntityInfo" then
        ...
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if hasPermission then
                print("noice")
                CopyToClipboard()
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.copied"))
            end
        end, "copyEntityInfo")
        print("no u")
    elseif info["action"] == "freeaimMode" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if hasPermission then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.freeaimTog"))
            end
        end, "freeaimMode")
    elseif info["action"] == "displayVehicles" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if hasPermission then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.dispVeh"))
            end
        end, "displayVehicles")
    elseif info["action"] == "displayPeds" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if hasPermission then
                ToggleEntityPedView()
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.dispPeds"))
            end
        end, "displayPeds")
    elseif info["action"] == "displayObjects" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if hasPermission then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong>"..Lang:t("alerts.dispEnt"))
            end
        end, "displayObjects")
    ...
    elseif info["action"] == "setweather" then
        if info["weather"] ~= "" and info["weather"] ~= nil then
            TriggerServerEvent("zk-admin:server:SetWeather", info["weather"])
        end
    ...
    elseif info["action"] == "superjump" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if AdminPanel.SuperJump then
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.enabled").."</strong> "..Lang:t("alerts.jumpOn"))
            else
                TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>DISABLED:</strong> "..Lang:t("alerts.jumpOff"))
            end
        end, "superjump")
    elseif info["action"] == "noragdoll" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if AdminPanel.NoRagdoll then
                ...
                TriggerEvent("zk-admin:client:ShowPanelAlert", "danger",  "<strong>"..Lang:t("alerts.disabled").."</strong>  "..Lang:t("alerts.jumpOff"))
            end
        end, "noragdoll")
    elseif info["action"] == "infinitestam" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            ...
            if AdminPanel.InfiniteStamina then
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.enabled").."</strong> "..Lang:t("alerts.infStamOn"))
            else
                TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.disabled").."</strong> "..Lang:t("alerts.infStamOff"))
            end
        end, "infinitestam")
    elseif info["action"] == "settime" then
        TriggerServerEvent("zk-admin:server:SetTime", info["time"], info["time"])
    elseif info["action"] == "reviveall" then
        TriggerServerEvent("zk-admin:server:ReviveAll")
    elseif info["action"] == "messageall" then
        if info["message"] then
            if #info["message"] > 5 then
                TriggerServerEvent("zk-admin:server:MessageAll", info["message"])
            else
                TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.msgShort"))
            end
        else
            TriggerEvent("zk-admin:client:ShowPanelAlert", "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.noMessage"))
        end
    elseif info["action"] == "removejob" then
        TriggerServerEvent("zk-admin:server:FireJob", currentPlayer)
    elseif info["action"] == "removegang" then
        TriggerServerEvent("zk-admin:server:FireGang", currentPlayer)
    elseif info["action"] == "savecar" then
        TriggerServerEvent("zk-admin:server:AddVehicleToGarage", currentPlayer)
    elseif info["action"] == "massdv" then
        TriggerServerEvent("zk-admin:server:DeleteAllEntities", 1) -- Vehicles
    elseif info["action"] == "massdp" then
        TriggerServerEvent("zk-admin:server:DeleteAllEntities", 2) -- Peds
    elseif info["action"] == "massdo" then
        TriggerServerEvent("zk-admin:server:DeleteAllEntities", 3) -- Objects
    elseif info["action"] == "clearreports" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if hasPermission then
                TriggerServerEvent("zk-admin:server:ClearJSON", "reports")
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.cleared").."</strong> "..Lang:t("alerts.reports"))
            end
        end, "clearreports")
    elseif info["action"] == "clearadminchat" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if hasPermission then
                TriggerServerEvent("zk-admin:server:ClearJSON", "admin")
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.cleared").."</strong> "..Lang:t("alerts.adminchat"))
            end
        end, "clearadminchat")
    elseif info["action"] == "clearlogs" then
        Compat.TriggerCallback("zk-admin:server:HasPermission", function(hasPermission)
            if hasPermission then
                TriggerServerEvent("zk-admin:server:ClearJSON", "logs")
                TriggerEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.cleared").."</strong> "..Lang:t("alerts.logs"))
            end
        end, "clearlogs")
    end
...
RegisterNUICallback("FirePlayerFromJob", function(citizenID)
    TriggerServerEvent("zk-admin:server:FireJobByCitizenId", citizenID)
    DebugTrace("NUICallback: FirePlayerFromJob ["..citizenID.."]")
end)

RegisterNUICallback("FirePlayerFromGang", function(citizenID)
    TriggerServerEvent("zk-admin:server:FireGangByCitizenId", citizenID)
...
RegisterNUICallback("SetJobGrade", function(data)
    if data["citizenid"] ~= nil and data["grade"] ~= nil then
        TriggerServerEvent("zk-admin:server:SetJobGradeByCitizenId", data["citizenid"], data["grade"])
...
RegisterNUICallback("SetGangGrade", function(data)
    if data["citizenid"] ~= nil and data["grade"] ~= nil then
        TriggerServerEvent("zk-admin:server:SetGangGradeByCitizenId", data["citizenid"], data["grade"])
...
RegisterNUICallback("UnbanPlayer", function(data)
    if data["license"] ~= nil then
        TriggerServerEvent("zk-admin:server:UnbanPlayer", data["license"])
    end
end)

RegisterNUICallback("DeleteReport", function(data)
    if data["id"] ~= nil then
        TriggerServerEvent("zk-admin:server:DeleteReport", data["id"])
    end
end)

RegisterNUICallback("ClaimReport", function(data)
    if data["id"] ~= nil then
        TriggerServerEvent("zk-admin:server:ClaimReport", data["id"])
    end
end)

RegisterNUICallback("DeleteCharacter", function(citizenId)
    if citizenId ~= nil then
        TriggerServerEvent("zk-admin:server:DeleteCharacter", citizenId)
    end
end)
    ...
    if data["setting"] ~= nil and data["value"] ~= nil then
        if data["setting"] == "darkmode" then
            if data["value"] == 1 then
                ...
                SetResourceKvpInt("zk-admin:setting:darkmode", 1)
            else
                AdminPanel.Settings.DarkMode = false
                SetResourceKvpInt("zk-admin:setting:darkmode", 0)
            end
        elseif data["setting"] == "seethrough" then
            if data["value"] == 1 then
                AdminPanel.Settings.SeeThrough = true
                SetResourceKvpInt("zk-admin:setting:seethrough", 1)
            else
                AdminPanel.Settings.SeeThrough = false
                SetResourceKvpInt("zk-admin:setting:seethrough", 0)
            end
        elseif data["setting"] == "Notifications" then
            if data["value"] == 1 then
                AdminPanel.Settings.Notifications = true
                SetResourceKvpInt("zk-admin:setting:notifications", 1)
            else
                AdminPanel.Settings.Notifications = false
                SetResourceKvpInt("zk-admin:setting:notifications", 0)
            end
        elseif data["setting"] == "theme" then
            AdminPanel.Settings.Notifications = tonumber(data["value"])
            SetResourceKvpInt("zk-admin:setting:theme", tonumber(data["value"]))
        end
...
RegisterNUICallback("Refresh", function()
    TriggerServerEvent("zk-admin:server:RefreshMenu")
end)
---------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------
function NoClip(on)
    ...
    while true do
        ...
        if AdminPanel.Spectating then
            ...
            if IsControlJustReleased(0, 174) then
                TriggerServerEvent("zk-admin:server:requestPrevSpectate")
            end
            if IsControlJustReleased(0, 175) then
                TriggerServerEvent("zk-admin:server:requestNextSpectate")
            end
        end
    end
end)
...
Citizen.CreateThread(function()
    while true do
        if AdminPanel.DisplayingBlips then
            local Players = {}
            Compat.TriggerCallback("zk-admin:server:GetPlayerPositions", function(p)
                Players = p
...
function getPlayerName(id)
    ...
    Compat.TriggerCallback("zk-admin:GetPlayerName", function(name)
        playerName = name
    end, id)
