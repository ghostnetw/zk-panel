...
RegisterNetEvent("zk-admin:server:ReportReply", function(data)
    local citizenid = nil
...
RegisterNetEvent("zk-admin:server:RequestPanel", function()
    local src = source
...
OpenPanel = function(source)
    ...
    AdminPanel.GetAllPlayers(function()
        ...
        local role = "Staff"
        TriggerClientEvent("zk-admin:client:OpenMenu", source, json.encode(ServerInformation.PlayerList), ServerInformation, GetConvarInt("sv_maxclients", 32), version, hasPerms, role)
    end)
end

Compat.CreateCallback("zk-admin:GetPlayerName", function(source, cb, id)
    local test = Compat.GetCharacterData(id)
    cb(test.CharacterName)
end)

Compat.CreateCallback("zk-admin:server:Refresh", function(source, cb)
    if AdminPanel.HasPermission(src, "adminmenu") then
        local playerList = {}
        for k, v in ipairs(GetPlayers()) do
            v = tonumber(v)
...
RegisterNetEvent("zk-admin:AddPlayer", function()
    local src = source
    TriggerClientEvent("zk-admin:AddPlayer", -1, src, os.time() )
end)

RegisterNetEvent("zk-admin:server:RequestJobPageInfo", function()
    local src = source
    if AdminPanel.HasPermission(src, "jobpage") then
        TriggerClientEvent("zk-admin:client:ReceiveJobPageInfo", src, Compat.GetMasterEmployeeList())
    end
end)

RegisterNetEvent("zk-admin:server:RequestGangPageInfo", function()
    local src = source
    if AdminPanel.HasPermission(src, "gangpage") then
        TriggerClientEvent("zk-admin:client:ReceiveGangPageInfo", src, Compat.GetMasterGangList())
    end
end)

RegisterNetEvent("zk-admin:server:RequestBansInfo", function()
    local src = source
...
RegisterNetEvent("zk-admin:server:ClearJSON", function(type)
    if type == "admin" then
        AdminPanel.AdminChat = {}
        SaveResourceFile(GetCurrentResourceName(), "json/adminchat.json", json.encode(AdminPanel.AdminChat), -1)
...
RegisterNetEvent("zk-admin:server:RequestReportsInfo", function()
    local src = source
    if AdminPanel.HasPermission(src, "viewreports") then
        TriggerClientEvent("zk-admin:client:ReceiveReportsInfo", src, AdminPanel.Reports)
    end
end)

RegisterNetEvent("zk-admin:server:RequestAdminChat", function()
    local src = source
    if AdminPanel.HasPermission(src, "adminchat") then
        TriggerClientEvent("zk-admin:client:ReceiveAdminChat", src, AdminPanel.AdminChat)
    end
end)

RegisterNetEvent("zk-admin:server:RequestVehiclesInfo", function()
    local src = source
    if AdminPanel.HasPermission(src, "vehiclesinfo") then
        local results = Compat.GetVehiclesList()
        if results ~= nil then
            TriggerClientEvent("zk-admin:client:ReceiveVehiclesInfo", src, results)
        end
    end
end)

RegisterNetEvent("zk-admin:server:RequestLeaderboardInfo", function()
    local src = source
    if AdminPanel.HasPermission(src, "leaderboardinfo") then
        local money, vehicles = Compat.GetLeaderboardInfo()
        TriggerClientEvent("zk-admin:client:ReceiveLeaderboardInfo", src, money, vehicles)
    end
end)

RegisterNetEvent("zk-admin:server:RequestItemsInfo", function()
    local src = source
    if AdminPanel.HasPermission(src, "itemsinfo") then
        local items = Compat.GetItemsList()
        TriggerClientEvent("zk-admin:client:ReceiveItemsInfo", src, items)
    end
end)

RegisterNetEvent("zk-admin:server:RequestCharacters", function()
    local src = source
...
RegisterNetEvent("zk-admin:server:RequestNoClip", function()
    local src = source
    if AdminPanel.HasPermission(src, "noclip") then
        TriggerClientEvent("zk-admin:client:ToggleNoClip", src)
    end
end)

RegisterNetEvent("zk-admin:server:AdminChatSend", function(message)
    local src = source
    if AdminPanel.HasPermission(src, "adminchat") then
        ...
        for k, v in pairs(GetPlayers()) do
            ...
            if AdminPanel.HasPermissionEx(v, "adminchat") then
                TriggerClientEvent("zk-admin:client:ReceiveAdminChat", v, AdminPanel.AdminChat)
            end
        end
    end
end)

RegisterNetEvent("zk-admin:server:SendReport", function(subject, info, type)
    local src = source
    ...
    if Player then
        ...
        if reportCount >= Config.MaxReportsPerPlayer then
            return TriggerClientEvent("zk-admin:client:ShowReportAlert", src, Lang:t("alerts.failedReportSend"), Lang:t("alerts.reportLimitReached"))
        end
        TriggerClientEvent("zk-admin:client:ShowReportAlert", src, Lang:t("alerts.reportSent"), "Your report was sent to server staff!")
        ...
        for k, v in pairs(GetPlayers()) do
            ...
            if AdminPanel.HasPermissionEx(v, "viewreports") then
                TriggerClientEvent("zk-admin:client:ShowReportAlert", v, Lang:t("alerts.newReport"), sendername..": "..subject.." Report ID: "..#AdminPanel.Reports)
            end
        end
    end
end)

RegisterServerEvent("zk-admin:server:ClaimReport", function(id)
    local src = source
        ...
        if AdminPanel.Reports[tonumber(id)] then
            ...
            for k, v in pairs(GetPlayers()) do
                ...
                if AdminPanel.HasPermission(v, "viewreports") then
                    TriggerClientEvent("zk-admin:client:ShowReportAlert", v, Lang:t("alerts.reportClaimed"), GetPlayerName(src).." claimed Report ID ".. id.." from "..AdminPanel.Reports[tonumber(id)].SenderName..".")
                end
            end
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", AdminPanel.Reports[tonumber(id)].SenderID, "success", "<strong>"..Lang:t("alerts.report").."</strong>"..Lang:t("alerts.reportClaimedByStaff").. "<strong>"..GetPlayerName(src).."</strong>.")
            TriggerClientEvent("zk-admin:client:ReceiveReportsInfo", src, AdminPanel.Reports)
        end
    end
end)
...
RegisterServerEvent("zk-admin:server:DeleteReport", function(id)
    local src = source
    if AdminPanel.HasPermission(src, "deletereport") then
        if AdminPanel.Reports[tonumber(id)] then
            ...
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>SUCCESS:</strong> Deleted Report ID " .. id..".")
            TriggerClientEvent("zk-admin:client:ReceiveReportsInfo", src, AdminPanel.Reports)
        end
    end
end)

RegisterNetEvent("zk-admin:server:ResourceAction", function(resourceName, action)
    local src = source
    if AdminPanel.HasPermission(src, "adminmenu") then
        if action == "start" then
            if GetResourceState(resourceName) == "stopped" then
                ...
                Wait(500)
                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.startedResource").."</strong> "..resourceName)
            else
                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.error").."</strong>"..Lang:t("alerts.resourceAlready"))
            end
        elseif action == "stop" then
            if GetResourceState(resourceName) == "started" then
                StopResource(resourceName)
                Wait(500)
                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.stoppedResource").."</strong> "..resourceName)                else
                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.error").."</strong>"..Lang:t("alerts.resouceStoppedAlready"))
            end
        elseif action == "restart" then
            if GetResourceState(resourceName) == "started" then
                ...
                Wait(500)
                StartResource(resourceName)
                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.RestartedResource").."</strong> "..resourceName)
            else
                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.error").."</strong>"..Lang:t("alerts.resouceRestartedAlready"))
            end
        end
        TriggerClientEvent("zk-admin:client:ForceReloadResources", src)
    end
end)

RegisterNetEvent("zk-admin:server:MonetaryAction", function(targetId, action, amount)
    local src = source
    if AdminPanel.HasPermission(src, "givetakemoney") then
        if action == "givecash" then
            Compat.PlayerActions.AddMoney(targetId, amount)
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", source, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.gaveCash", {value = amount, value2 = GetPlayerName(targetId)}))
        elseif action == "removecash" then
            Compat.PlayerActions.RemoveMoney(targetId, amount)
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", source, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.removeCash", {value = amount, value2 = GetPlayerName(targetId)}))
        elseif action == "givebank" then
            Compat.PlayerActions.AddBank(targetId, amount)
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", source, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.gaveBank", {value = amount, value2 = GetPlayerName(targetId)}))
        elseif action == "removebank" then
            Compat.PlayerActions.RemoveBank(targetId, amount)
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", source, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.removeBank", {value = amount, value2 = GetPlayerName(targetId)}))
        end
    end
end)

RegisterNetEvent("zk-admin:server:RequestViewPlayer", function(CitizenId)
    ...
    if Player then
        ...
        TriggerClientEvent("zk-admin:client:ViewPlayer", src, true, sourcer)
    else
        TriggerClientEvent("zk-admin:client:ViewPlayer", src, false, Compat.GetOfflinePlayerFromCharacterIdentifier(CitizenId))
    end
end)

AddEventHandler("playerDropped", function()
	local src = source
    TriggerClientEvent("zk-admin:RemovePlayer", -1, src)
end)

RegisterNetEvent("zk-admin:server:RefreshMenu", function(silent)
    local src = source
    TriggerClientEvent("zk-admin:client:RefreshMenu", src, json.encode(Compat.GetPlayerList()), silent)
end)
...
Compat.CreateCallback("zk-admin:server:HasPermission", function(source, cb, permission)
...
AdminPanel.HasPermission = function(targetId, permName)
    ...
    if hasPerms then
        if permName ~= "clearreports" and permName ~= "clearadminchat" then
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", targetId, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.noPermission"))
        end
        TriggerClientEvent("zk-admin:client:ResetMenu", targetId)
    end
    return false
end
...
RegisterNetEvent("zk-admin:server:RequestResourcePageInfo", function()
    local src = source
    if AdminPanel.HasPermission(src, "resourcepage") then
        ...
        TriggerClientEvent("zk-admin:client:ReceiveResourcePageInfo", src, resourceList)
    end
end)

RegisterNetEvent("zk-admin:server:RequestCurrentLogs", function()
    local src = source
    if AdminPanel.HasPermission(src, "serverlogs") then
        TriggerClientEvent("zk-admin:client:ReceiveCurrentLogs", src, InterceptedLogs)
    end
end)

RegisterNetEvent("zk-admin:server:OpenInventory", function(target)
    TriggerClientEvent("inventory:client:RobPlayer:Admin", target, tonumber(target))
end)

RegisterNetEvent("zk-admin:server:RequestServerMetrics", function()
    local src = source
    if AdminPanel.HasPermission(src, "servermetrics") then
        ...
        TriggerClientEvent("zk-admin:client:ReceiveServerMetrics", src, ServerMetrics)
    end
end)
...
RegisterNetEvent("zk-admin:server:GetPlayersForBlips", function()
    local src = source
    if Config.EnableNames then
        ...
        TriggerClientEvent("zk-admin:client:Show", src, PlayersB)
    end
end)