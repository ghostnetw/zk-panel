...
RegisterServerEvent("zk-admin:server:SetPosition", function(playerId, x, y, z)
    local src = source
...
RegisterServerEvent("zk-admin:server:KillPlayer", function(playerId)
    local src = source
    if AdminPanel.HasPermission(src, "kill") then
        TriggerClientEvent("hospital:client:KillPlayer", playerId)
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.killed").." " .. GetPlayerName(playerId) .. ".")
...
RegisterServerEvent("zk-admin:server:SavePlayer", function(playerId)
    local src = source
    if QBCore then
        if AdminPanel.HasPermission(src, "savedata") then
            local TargetPlayer = QBCore.Functions.GetPlayer(playerId)
            TargetPlayer.Functions.Save()
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.savedToDB", {value = GetPlayerName(playerId)}))
...
RegisterServerEvent("zk-admin:server:KickPlayer", function(playerId, reason)
    local src = source
...
RegisterServerEvent("zk-admin:server:Freeze", function(playerId)
    local src = source
    if AdminPanel.HasPermission(src, "freeze") then
        if not Frozen[playerId] then
            ...
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.froze").." " .. GetPlayerName(playerId) .. ".")
            ...
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.unfroze").." " .. GetPlayerName(playerId) .. ".")
...
RegisterServerEvent("zk-admin:server:KickAllPlayers", function(reason)
    local src = source
...
RegisterNetEvent("zk-admin:server:BanPlayer", function(player, time, reason, citizenid)
    local src = source
            ...
            if citizenid ~= nil then
                ...
                if result[1] then
                    ...
                    if not online then
                        ...
                        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "Banned " .. result[1].name .. " (OFFLINE) for " .. (time / 60 / 60) .. " hours.")
...
AdminPanel.OnlineBanPlayer = function(source, player, time, timeTable, reason)
    ...
    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "Banned " .. GetPlayerName(player) .. " for " .. (time / 60 / 60) .. " hours.")
    if Config.AnnounceBan then
        TriggerClientEvent("chat:addMessage", -1, {
...
RegisterNetEvent("zk-admin:server:WarnPlayer", function(player, reason, citizenid)
    local src = source
            ...
            if citizenid ~= nil then
                ...
                if result[1] then
                    ...
                    if not online then
                        ...
                        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.warnedPlayerOffline", {value = result[1].name}))
...
AdminPanel.OnlineWarnPlayer = function(source, player, reason)
    ...
    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.warnedPlayerOnline", {value = GetPlayerName(player)}))
    ...
    TriggerClientEvent("zk-admin:client:WarnPlayer", player, GetPlayerName(src), reason)
end

RegisterNetEvent("zk-admin:server:ViewWarnings", function(player, citizenid)
    local src = source
    local license = nil
    if AdminPanel.HasPermission(src, "checkwarns") then
        if player ~= "OFFLINE" then
            ...
            DebugTrace("[zk-admin:server:ViewWarnings] Got license (online): " .. license)
        else
            if citizenid ~= nil then
                local result = MySQL.query.await("SELECT * FROM `" .. Config.DB.CharactersTable .. "` WHERE `citizenid` = ?", {citizenid})
                if result[1] then
                    license = result[1].license
                    DebugTrace("[zk-admin:server:ViewWarnings] Got license (offline): " .. license)
                else
        ...
        if license ~= nil then
            ...
            if #result > 0 then
                TriggerClientEvent("zk-admin:client:ViewWarnings", src, result)
                DebugTrace("[zk-admin:server:ViewWarnings] Sending warnings")
            else
                if player ~= "OFFLINE" then
                    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.noWarnings").."</strong> "..Lang:t("alerts.noWarningsPlayer"))
                else
                    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.noWarnings").."</strong> "..Lang:t("alerts.noWarningsPlayer"))
                end
            end
...
RegisterNetEvent("zk-admin:server:CuffPlayer", function(target)
    local src = source
    if AdminPanel.HasPermission(src, "cuff") then
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.cuffed"))
...
RegisterServerEvent("zk-admin:server:RevivePlayer", function(target)
    local src = source
    if AdminPanel.HasPermission(src, "revive") then
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.revivePlayer", {value = GetPlayerName(target)}))
...
RegisterServerEvent("zk-admin:server:ReviveAll", function()
    local src = source
    if AdminPanel.HasPermission(src, "reviveall") then
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.revivedAll"))
...
RegisterServerEvent("zk-admin:server:MessageAll", function(message)
    local src = source
    if AdminPanel.HasPermission(src, "messageall") then
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.sentMessageAll"))
        TriggerClientEvent("chat:addMessage", -1, {
            color = {255, 50, 50},
            multiline = true,
            args = {"SYSTEM", message}
...
RegisterServerEvent("zk-admin:server:DeleteAllEntities", function(entityType)
    local src = source
    ...
    if AdminPanel.HasPermission(src, "massdeleteentities") then
        ...
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.DeletedAllEntities", {value = entityTypeString}))
        TriggerClientEvent("zk-admin:client:DeleteAllEntities", -1, entityType)
    end
end)

RegisterServerEvent("zk-admin:server:SetWeather", function(weatherType)
    local src = source
    if AdminPanel.HasPermission(src, "weather") then
        if QBCore then
            exports["qb-weathersync"]:setWeather(weatherType)
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.setWeather", {weatherType}))
        elseif ESX then
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "error", "<strong>"..Lang:t("alerts.error").."</strong> This feature is currently unavailable!")
        end
    end
end)

RegisterServerEvent("zk-admin:server:SetTime", function(hour, minute)
    local src = source
    if AdminPanel.HasPermission(src, "time") then
        if QBCore then
            exports["qb-weathersync"]:setTime(hour, 0)
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.setTime", {value = hour}))
        elseif ESX then
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "error", "<strong>"..Lang:t("alerts.error").."</strong> This feature is currently unavailable!")
        end
    end
...
RegisterServerEvent("zk-admin:server:FeedPlayer", function(target)
    local src = source
    if AdminPanel.HasPermission(src, "foodandwater") then
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.setMaxValues"))
        TriggerEvent("zk-admin:server:SetMetaDataForPlayer", target, "thirst", 100)
        TriggerEvent("zk-admin:server:SetMetaDataForPlayer", target, "hunger", 100)
        if QBCore then
...
RegisterServerEvent("zk-admin:server:RelieveStress", function(target)
    local src = source
    if AdminPanel.HasPermission(src, "relievestress") then
        if QBCore then
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.stressRelieved"))
            TriggerEvent("zk-admin:server:SetMetaDataForPlayer", target, "stress", 0)
...
RegisterServerEvent("zk-admin:server:SetMetaDataForPlayer", function(PlayerId, Meta, Data)
...
RegisterNetEvent("zk-admin:server:SetPedModel", function(player, model)
    local src = source
    if AdminPanel.HasPermission(src, "setpedmodel") then
        TriggerClientEvent("zk-admin:client:SetPedModel", player, model)
    end
end)

RegisterNetEvent("zk-admin:server:RequestSpectate", function(player)
    local src = source
    if AdminPanel.HasPermission(src, "spectate") then
        local coords = GetEntityCoords(GetPlayerPed(player))
        SpectatingPlayer[src] = player
        TriggerClientEvent("zk-admin:client:RequestSpectate", src, player, coords, GetPlayerName(player))
...
RegisterNetEvent("zk-admin:server:requestNextSpectate", function()
    local src = source
        ...
        repeat
            ...
            if Player ~= nil then
                if i == src then
                    ...
                    TriggerClientEvent("zk-admin:client:RequestSpectate", src, i, coords, GetPlayerName(i))
...
RegisterNetEvent("zk-admin:server:requestPrevSpectate", function()
    local src = source
    if AdminPanel.HasPermission(src, "spectate") and SpectatingPlayer[src] then
        local foundPlayer = false
        local i = SpectatingPlayer[src] - 1
...
RegisterNetEvent("zk-admin:server:ScreenshotSubmit", function(playerId)
    local src = source
    if AdminPanel.HasPermission(src, "screenshot") then
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.screenshotting"))
...
RegisterNetEvent("zk-admin:server:SaveCar", function(mods, vehicle, hash, plate, senderId)
    local src = source
    ...
    if result[1] == nil then
        ...
        if senderId then
            ...
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", senderId, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.addedVehicle", {value = vehicle.model}))
        end
    else
        if senderId then
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", senderId, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.playerOwnsAlready"))
        end
        if QBCore then
...
RegisterServerEvent("zk-admin:server:RequestVehicleSpawn", function(modelName)
    local src = source
...
RegisterServerEvent("zk-admin:server:DeleteCharacter", function(citizenId)
    local src = source
            ...
            if result[1] then
                MySQL.query("DELETE FROM `" .. Config.DB.CharactersTable .. "` WHERE citizenid = ? LIMIT 1", {citizenId}, function(rowsAffected)
                    if rowsAffected then
                        ...
                        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.deletedCharacter"))
                        ...
                        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.noRowsDeleted"))
                    end
                    local results = MySQL.query.await("SELECT * FROM `"..Config.DB.CharactersTable.."`")
                    TriggerClientEvent("zk-admin:client:ReceiveCharacters", src, results)
                end)
            else
                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.cantFindLicense"))
            end
        end)
    end
end)
...
RegisterServerEvent("zk-admin:server:UnbanPlayer", function(license)
    local src = source
        ...
        MySQL.query("SELECT * FROM `bans` WHERE license = ? LIMIT 1", {license}, function(result)
            if result[1] then
                MySQL.query("DELETE FROM `bans` WHERE license = ? LIMIT 1", {license}, function(rowsAffected)
                    if rowsAffected then
                        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.unbanned"))
                    ...
                    TriggerClientEvent("zk-admin:client:ReceiveBansInfo", src, BansInfo)
                end)
            else
                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.cantFindLicense"))
            end
...
RegisterServerEvent("zk-admin:server:ClearInventory", function(targetId)
    local src = source
    if AdminPanel.HasPermission(src, "clearinventory") then
        ...
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.clearedInventory"))
    end
end)

RegisterServerEvent("zk-admin:server:SetJob", function(targetId, job, grade)
    local src = source
    if AdminPanel.HasPermission(src, "setjob") then
        ...
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.jobSet", {value = job, value2 = grade}))
    end
end)

RegisterServerEvent("zk-admin:server:SetGang", function(targetId, gang, grade)
    local src = source
            ...
            if QBCore.Shared.Gangs[gang] ~= nil then
                if QBCore.Shared.Gangs[gang].grades[tostring(grade)] ~= nil then
                    ...
                    if targetPlayer then
                        ...
                        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.jobSet", {value = gang, value2 = grade}))
                    end
                else
                    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.invalidgangGrade"))
                end
            else
                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.invalidgang"))
            end
        end
    end
end)

RegisterServerEvent("zk-admin:server:GiveItem", function(targetId, item, amount)
    local src = source
        ...
        if QBCore then
            if QBCore.Shared.Items[item] ~= nil then
                ...
                if targetPlayer then
                    ...
                    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.gaveItem", {value = item}))
                    TriggerClientEvent("QBCore:Notify", targetId, Lang:t("notify.givenItem", {value = item}), "success")
                end
            else
                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.invalidItem"))
            end
        elseif ESX then
...
RegisterServerEvent("zk-admin:server:FireJob", function(targetId)
    local src = source
    if AdminPanel.HasPermission(src, "firejob") then
        ...
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.firedJob"))
    end
end)

RegisterServerEvent("zk-admin:server:FireGang", function(targetId)
    local src = source
    if AdminPanel.HasPermission(src, "firegang") then
        ...
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.firedGang"))
    end
end)

RegisterServerEvent("zk-admin:server:FireJobByCitizenId", function(citizenId)
    local src = source
    if AdminPanel.HasPermission(src, "firejob") then
...
RegisterServerEvent("zk-admin:server:FireGangByCitizenId", function(citizenId)
    local src = source
    if AdminPanel.HasPermission(src, "firegang") then
        if QBCore then
...
RegisterServerEvent("zk-admin:server:SetGangGradeByCitizenId", function(citizenId, grade)
    local src = source
    if AdminPanel.HasPermission(src, "setgang") then
        if QBCore then
            local targetPlayer = QBCore.Functions.GetPlayerByCitizenId(citizenId)
            if targetPlayer then
                if QBCore.Shared.Gangs[targetPlayer.PlayerData.gang.name].grades[grade] ~= nil then
                    ...
                    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> Set gang grade of " .. AdminPanel.CharacterName(targetPlayer) .. " (" .. GetPlayerName(targetId) .. ") to " .. grade)
                else
                    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.invalidgangGrade"))
                end
            else
                ...
                if result ~= nil then
                    ...
                    if gangInfo.grade ~= nil then
                        if QBCore.Shared.Gangs[gangInfo.name].grades[grade] ~= nil then
                            ...
                            MySQL.update("UPDATE `" .. Config.DB.CharactersTable .. "` SET `gang` = ? WHERE `citizenid` = ?", {json.encode(gangInfo), citizenId}, function(rowsAffected)
                                if rowsAffected ~= 0 and rowsAffected ~= nil then
                                    ...
                                    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.gangGradeSet", {value = grade}))
                                else
                                    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.databaseError"))
                                end
                            end)
                        else
                            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.invalidgangGrade"))
                        ...
                        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.databaseError2"))
                    end
                else
                    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.databaseError3"))
                end
            end
        end
    end
...
RegisterServerEvent("zk-admin:server:SetJobGradeByCitizenId", function(citizenId, grade)
    local src = source
    if AdminPanel.HasPermission(src, "setjob") then
        local targetPlayer = Compat.GetPlayerFromCharacterIdentifier(citizenId)
        if targetPlayer then
            if QBCore then
                if QBCore.Shared.Jobs[targetPlayer.PlayerData.job.name].grades[grade] ~= nil then
                    ...
                    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> Set job grade of " .. AdminPanel.CharacterName(targetPlayer) .. " (" .. GetPlayerName(targetId) .. ") to " .. grade)
                else
                    TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> Invalid job grade.")
                end
            elseif ESX then
                    ...
                    if QBCore.Shared.Jobs[jobInfo.name].grades[grade] ~= nil then
                        ...
                        MySQL.update("UPDATE `" .. Config.DB.CharactersTable .. "` SET `job` = ? WHERE `citizenid` = ?", {json.encode(jobInfo), citizenId}, function(rowsAffected)
                            if rowsAffected ~= 0 and rowsAffected ~= nil then
                                ...
                                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.jobGradeSet", {value = grade}))
                            else
                                TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.databaseError"))
                            end
                        end)
                    else
                        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.invalidJobGrade"))
                    end
                elseif ESX then
...
RegisterServerEvent("zk-admin:server:AddVehicleToGarage", function(targetId)
    local src = source
    if AdminPanel.HasPermission(src, "savecar") then
        TriggerClientEvent("zk-admin:client:SaveCar", targetId, src)
    end
end)

RegisterServerEvent("zk-admin:server:BringPlayer", function(targetId)
    local src = source
    if AdminPanel.HasPermission(src, "teleport") then
        SendPlayerBack[targetId] = GetEntityCoords(GetPlayerPed(targetId))
        local coords = GetEntityCoords(GetPlayerPed(src))
        SetEntityCoords(GetPlayerPed(targetId), coords)
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.teleportToYou", {value = GetPlayerName(targetId)}))
...
RegisterServerEvent("zk-admin:server:SendPlayerBack", function(targetId)
    local src = source
    if AdminPanel.HasPermission(src, "teleport") then
        if SendPlayerBack[targetId] then
            SetEntityCoords(GetPlayerPed(targetId), SendPlayerBack[targetId])
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.teleportedBack", {value = GetPlayerName(targetId)}))
            SendPlayerBack[targetId] = nil
        else
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.noPosition"))
        end
    end
end)

RegisterServerEvent("zk-admin:server:SendBackSelf", function()
    local src = source
    if AdminPanel.HasPermission(src, "teleport") then
        if SendBack[src] then
            SetEntityCoords(GetPlayerPed(src), SendBack[src])
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.sentSelfBack"))
            SendBack[src] = nil
        else
            TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "danger", "<strong>"..Lang:t("alerts.error").."</strong> "..Lang:t("alerts.noPosition"))
        end
...
RegisterServerEvent("zk-admin:server:GotoPlayer", function(targetId)
    local src = source
    if AdminPanel.HasPermission(src, "teleport") then
        ...
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", src, "success", "<strong>"..Lang:t("alerts.success").."</strong> " ..Lang:t("alerts.teleportedTo", {value = GetPlayerName(targetId)}))
...
if QBCore then
    QBCore.Functions.CreateCallback("zk-admin:server:HasPermissions", function(source, cb, group)
        local src = source
        local retval = false
    ...
    QBCore.Functions.CreateCallback("zk-admin:server:GetPlayerPositions", function(source, cb)
...
elseif ESX then
    ESX.RegisterServerCallback("zk-admin:server:HasPermissions", function(source, cb, group)
        local src = source
        local retval = false
    ...
    ESX.RegisterServerCallback("zk-admin:server:GetPlayerPositions", function(source, cb)
        local PlayerPositions = {}
        local Trigger = nil
        for k, v in pairs(ESX.GetPlayers()) do
            local Player = nil
...
RegisterServerEvent("zk-admin:server:SetPermissions", function(targetId, group)
    if QBCore then
...
RegisterServerEvent("zk-admin:server:OpenSkinMenu", function(targetId)
    local src = source
    if AdminPanel.HasPermission(src, "skinmenu") then
        TriggerClientEvent("zk-admin:client:ShowPanelAlert", source, "success", "<strong>"..Lang:t("alerts.success").."</strong> "..Lang:t("alerts.skinMenuOpened", {value = GetPlayerName(targetId)}))
        if QBCore then
            TriggerClientEvent("qb-clothing:client:openMenu", targetId)
        elseif ESX then
            TriggerClientEvent("zk-admin:client:OpenSkinMenu1", targetId)
        end
